...
class CommonHelper extends AppHelper {
	...
	public function flash(array $types = array()) {
		...
		$html = '';
		if (!empty($messages)) {
			$html = '<div id="flash-messages" class="row"><div class="col-md-8 col-md-offset-2">';
	...
	public function flashMessage($msg, $type = 'info', $escape = true) {
		$html = '<div id="flash-message" class="row"><div class="col-md-8 col-md-offset-2">';
		if ($escape) {
			$msg = h($msg);
		}
	...
	protected function _message($msg, $type) {
		if (!empty($msg)) {
			return '<div class="alert fade in' . (!empty($type) ? ' alert-' . $type : '') . ' alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>' . $msg . '</div>';
		}
		return '';
	}
