 ...
 * - actsAs = array("Tools.Loggable"); on models that should be logged
 ...
 * - "version_id"    [int]     : cooperates with RevisionBehavior to link the the shadow table (thus linking to old
 * data) Remember that Loggable behavior needs to be added after RevisionBehavior. In fact, just put it last to be
 * safe. Optionally register what user was responsible for the activity :
 * - Supply configuration only if defaults are wrong. Example given with defaults :
 *        public $actsAs = array('Tools.Loggable' => array('userModel' => 'User', 'userKey' => 'user_id'));
 * - In AppController (or single controller if only needed once) add these lines to beforeFilter :
 *    if (count($this->uses) && $this->{$this->modelClass}->Behaviors->loaded('Loggable')) {
 *            $this->{$this->modelClass}->setUserData($this->activeUser);
 *        }
 * Where "$activeUser" should be an array in the standard format for the User model used :
 * $activeUser = array( $UserModel->alias => array( $UserModel->primaryKey => 123, $UserModel->displayField =>
 * 'Alexander'));
...
class LoggableBehavior extends ModelBehavior {
	...
	protected $_defaultConfig = array(
		...
		'userKey'        => 'user_id',
		'change'         => 'serialize',
		'descriptionIds' => false,
		'skip'           => array( 'index', 'view' ),
		'ignore'         => array( 'password', 'created', 'created_by', 'modified', 'modified_by' ),
		'classField'     => 'model',
	...
	/**
	 * Config options are :
	 * - userModel : 'User'. Class name of the user model you want to use (User by default), if you want to save
	 * User in log
	...
	public function setup( Model $Model, $config = array() ) {
		$config += (array) Configure::read( 'Loggable' );
	...
	public function findUserActions( Model $Model, $userId, $params = array() ) {
		...
		if ( isset( $this->user ) && isset( $this->user[ $this->UserModel->alias ][ $this->UserModel->primaryKey ] ) && $userId == $this->user[ $this->UserModel->alias ][ $this->UserModel->primaryKey ] && isset( $this->user[ $this->UserModel->alias ][ $this->UserModel->displayField ] )
			...
			$user                       = $this->UserModel->find( 'first',
				array( 'conditions' => array( $this->UserModel->primaryKey => $userId ) ) );
		...
		foreach ( $data as $key => $row ) {
			...
			if ( isset( $one[ $this->settings[ $Model->alias ]['classField'] ] ) && isset( $one['action'] ) && isset( $one['change'] ) && isset( $one[ $this->settings[ $Model->alias ]['foreignKey'] ] )
				...
				if ( $one['action'] === 'edit' ) {
					$result[ $key ][ $this->Log->alias ]['event'] .= ' edited ' . $one['change'] . ' of ' . strtolower( $one[ $this->settings[ $Model->alias ]['classField'] ] ) . '(id ' . $one[ $this->settings[ $Model->alias ]['foreignKey'] ] . ')';
				} elseif ( $one['action'] === 'add' ) {
					$result[ $key ][ $this->Log->alias ]['event'] .= ' added a ' . strtolower( $one[ $this->settings[ $Model->alias ]['classField'] ] ) . '(id ' . $one[ $this->settings[ $Model->alias ]['foreignKey'] ] . ')';
				} elseif ( $one['action'] === 'delete' ) {
					$result[ $key ][ $this->Log->alias ]['event'] .= ' deleted the ' . strtolower( $one[ $this->settings[ $Model->alias ]['classField'] ] ) . '(id ' . $one[ $this->settings[ $Model->alias ]['foreignKey'] ] . ')';
				}
				...
				if ( $one['action'] === 'edit' ) {
					$result[ $key ][ $this->Log->alias ]['event'] .= ' edited ' . strtolower( $one[ $this->settings[ $Model->alias ]['classField'] ] ) . '(id ' . $one[ $this->settings[ $Model->alias ]['foreignKey'] ] . ')';
				} elseif ( $one['action'] === 'add' ) {
					$result[ $key ][ $this->Log->alias ]['event'] .= ' added a ' . strtolower( $one[ $this->settings[ $Model->alias ]['classField'] ] ) . '(id ' . $one[ $this->settings[ $Model->alias ]['foreignKey'] ] . ')';
				} elseif ( $one['action'] === 'delete' ) {
					$result[ $key ][ $this->Log->alias ]['event'] .= ' deleted the ' . strtolower( $one[ $this->settings[ $Model->alias ]['classField'] ] ) . '(id ' . $one[ $this->settings[ $Model->alias ]['foreignKey'] ] . ')';
	...
	protected function _saveLog( Model $Model, $logData, $title = null ) {
		...
		if ( $this->Log->hasField( 'description' ) ) {
			...
			if ( $this->user && $this->UserModel && isset( $this->user[ $this->UserModel->alias ] ) ) {
				$logData['description'] .= ' ' . __( 'by' ) . ' ' . $this->user[ $this->UserModel->alias ][ $this->UserModel->displayField ];
	...
	/**
	 * LoggableBehavior::clearUserData()
	 *
	 * @param Model $Model
	 *
	 * @return void
	 ...
	 *    if ($this->{$this->modelClass}->Behaviors->loaded('Loggable')) {
	...
	/**
	 * LoggableBehavior::beforeValidate()
	 *
	 * @param Model $Model
	 * @param array $options
	 *
	...
	public function afterSave( Model $Model, $created, $options = array() ) {
		...
		if ( $this->Log->hasField( 'description' ) ) {
			...
			if ( $created ) {
				...
				// Has the row actually been archived, rather than edited? - 1
				if ( in_array( 'archived_date', $diff ) ) {
					$logData['description'] .= __( 'archived' );
				} else {
		...
		if ( $this->Log->hasField( 'action' ) ) {
			if ( $created ) {
				...
				// Has the row actually been archived, rather than edited? - 2
				if ( in_array( 'archived_date', $diff ) ) {
					$logData['action'] = 'archive';
				} else {
					$logData['action'] = 'edit';
				}
			}
		...
		if ( $this->Log->hasField( 'change' ) ) {
			...
			foreach ( $Model->data[ $Model->alias ] as $key => $value ) {
				...
				if ( $key !== 'modified' && ! in_array( $key,
						$this->settings[ $Model->alias ]['ignore'] ) && $value != $old && in_array( $key, $dbFields )
				) {
